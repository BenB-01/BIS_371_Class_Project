...
# import pyodbc allowing certain tools like cursor and connect
import pyodbc 

#put connection information into variable connect string

connStr = (r"DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};" r"DBQ=C:Users\clarkena\Desktop\BIS371_project1.accdb;")# connect to database
try:
...
def submitted_data_find():
  ...
  for row in rows:
    display_textbox.insert("1.0", ("Submitted: " + '{:%Y-%m-%d}'.format(row.Activity_date) + '\n'))
...
def accepted_data_find():
  ...
  rows = cursor.fetchall()
  for row in rows:
    display_textbox.insert("1.0", ("Accepted: " + '{:%Y-%m-%d}'.format(row.Activity_date) + '\n'))
...
def rejected_data_find():
  ...
  rows = cursor.fetchall()
  for row in rows:
    display_textbox.insert("1.0", ("Rejected: " + '{:%Y-%m-%d}'.format(row.Activity_date) + '\n'))
...
def rr_data_find():
  ...
  rows = cursor.fetchall()
  for row in rows:
    display_textbox.insert("1.0", ("R&R: " + '{:%Y-%m-%d}'.format(row.Activity_date) + '\n'))
